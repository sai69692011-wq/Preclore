...
function presencePoints(value, points) {
  return String(value || '').trim() ? points : 0;
}
...
function evidenceScore({ projectTag, evidenceUrls, citations, reproducibilityNote, projectDocumentUrl }) {
  ...
  const documentBonus = String(projectDocumentUrl || '').trim() ? 4 : 0;
  const citationsPoints = lengthScore(citations, 10, 35, 5);
  const reproducibilityPoints = lengthScore(reproducibilityNote, 12, 40, 5);

  return clamp(
    Math.min(urls.length, 4) * 2 +
      fieldVerifiedBonus +
      documentBonus +
      citationsPoints +
      reproducibilityPoints,
    0,
    20
  );
}
...
export function scoreProject(input = {}) {
  ...
  const foundation = clamp(
    presencePoints(form.title, 8) +
      presencePoints(form.summary, 8) +
      presencePoints(projectTag, 4) +
      presencePoints(form.projectDocumentUrl || form.pdfUrl, 4),
    0,
    20
  );

  const clarity = clamp(
    lengthScore(form.summary, 25, 120, 14) +
      (String(form.projectDocumentUrl || form.pdfUrl || '').trim() ? 6 : 0),
    0,
    20
  );
  ...
  const publicGood = clamp(
    lengthScore(form.publicGoodCase, 20, 75, 12) +
      (projectTag === 'Field Verified' ? 2 : 0) +
      (projectTag === 'Project: Needs Funding' || projectTag === 'Needs Funding' ? 2 : 0),
    0,
    20
  );
