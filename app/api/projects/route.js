...
import { PROJECT_SLOT_LIMIT, PROJECT_TAG_COMPAT } from '@/lib/constants';
...
export async function POST(request) {
  ...
  if (!user) {
    return NextResponse.json({ error: 'Please login before adding a project.' }, { status: 401 });
  }
  ...
  if (body.confirmPublicGood !== true) {
    return NextResponse.json(
      { error: 'Please confirm that this project can be shown publicly.' },
      { status: 400 }
    );
  }
  ...
  if (missing) {
    return NextResponse.json({ error: 'Please fill in all required project details.' }, { status: 400 });
  }
  ...
  if (!access.canSubmit) {
    return NextResponse.json(
      { error: 'This account cannot add projects.' },
      { status: 403 }
    );
  }
  ...
  if ((activeProjectCount || 0) >= PROJECT_SLOT_LIMIT) {
    return NextResponse.json(
      {
        error: `You already used all ${PROJECT_SLOT_LIMIT} active project slots. Archive one project from your profile before adding a new one.`
      },
      { status: 400 }
    );
  }
  ...
  if (!publicName) {
    return NextResponse.json(
      { error: 'Complete your profile with your public name before adding a project.' },
      { status: 400 }
    );
  }
  ...
  return NextResponse.json({
    ...
    score,
    slots: {
      limit: PROJECT_SLOT_LIMIT,
      used: (activeProjectCount || 0) + 1
    }
  });
}
