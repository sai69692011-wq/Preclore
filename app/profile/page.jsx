...
export default async function ProfilePage() {
    ...
    return (
      <div className="rounded-[30px] border-2 border-ink bg-white/80 p-8 shadow-[0_8px_0_0_rgba(44,43,42,1)]">
        ...
        <p className="mt-3 text-sm leading-7 text-ink/80">
          Login or create your account first to edit your profile.
        </p>
        <div className="mt-5">
  ...
  const [{ data: profile }, { data: projects }] = await Promise.all([
    ...
    supabase
      .from('projects')
      .select('id, slug, title, summary, project_tag, status, published_at')
      .eq('researcher_id', user.id)
      .order('published_at', { ascending: false })
  ]);

  return <ProfileForm initialProfile={profile} initialProjects={projects || []} />;
}
